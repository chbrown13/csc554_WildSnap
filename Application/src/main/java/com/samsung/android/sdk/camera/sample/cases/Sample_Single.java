...
import com.samsung.android.sdk.camera.sample.CameraSDK_Sample;
...
public class Sample_Single extends Activity implements SettingDialog.OnCameraSettingUpdatedListener {
    ...
    public static ArrayList<Image> takenImages = new ArrayList<Image>();
    ...
    private ImageReader.OnImageAvailableListener mImageCallback = new ImageReader.OnImageAvailableListener() {
        ...
        public void onImageAvailable(ImageReader reader) {
            if(mImageFormat == ImageFormat.JPEG)
            {
                ...
                mImageSaver.save(image, fileName);
            }
            else
            {
                mImageSaver.save(reader.acquireNextImage(), createFileName() + ".dng");
            }
        }
    };
    ...
    private void createUI() {
        ...
        findViewById(R.id.picture).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                TextView t = (TextView) findViewById(R.id.save_pics);
                t.setText("Save");
                Button b = (Button)findViewById(R.id.save_pics);
                ...
                if (getState() == CAMERA_STATE.PREVIEW) {
                    ...
                    } else if (mPreviewBuilder.get(SCaptureRequest.CONTROL_AF_MODE) != SCaptureRequest.CONTROL_AE_MODE_OFF &&
                            mCharacteristics.get(SCameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL) != SCameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY) {
                        triggerAE();
        ...
        findViewById(R.id.save_pics).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                TextView t = (TextView) findViewById(R.id.save_pics);
                CharSequence cs = t.getText();
                if(cs.equals("Save")) {
                    startActivity(new Intent(Sample_Single.this, SavePicturePage.class));
                }
                else if (cs.equals("Cancel")) {
                    // go straight to profile screen
                    startActivity(new Intent(Sample_Single.this, ProfilePage.class));
                }
            }
        });
    ...
    synchronized private void openCamera(int facing) {
        try {
            ...
            Size jpegSize = streamConfigurationMap.getOutputSizes(ImageFormat.JPEG)[0];
            ...
            if(contains(mCharacteristics.get(SCameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES), SCameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES_RAW)) {
                Size rawSize = streamConfigurationMap.getOutputSizes(ImageFormat.RAW_SENSOR)[0];
    ...
    synchronized private void createPreviewSession() {
        ...
        try {
            ...
            mPreviewBuilder = mSCameraDevice.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW);
            mPreviewBuilder.addTarget(surface);

            // Creates SCaptureRequest.Builder for still capture with output target.
            mCaptureBuilder = mSCameraDevice.createCaptureRequest(CameraDevice.TEMPLATE_STILL_CAPTURE);
            ...
            mSCameraSession.capture(mCaptureBuilder.build(), new SCameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(SCameraCaptureSession session, SCaptureRequest request, STotalCaptureResult result) {
                    ...
                    try {
                        ...
                        //TODO iv.setImageResource(R.drawable.image_name);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                ...
                new Handler(Looper.myLooper()).post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                           public void run() {
                                //Toast.makeText(Sample_Single.this, "Stored for saving: " + file.getName(), Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                });
